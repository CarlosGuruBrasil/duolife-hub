...
interface PortalCotacoesPaginationProps {
  currentPage: number;
  totalPages: number;
  totalRecords: number;
  pageSize: number;
...
export function PortalCotacoesPagination({
  currentPage,
  totalPages,
  totalRecords,
  pageSize,
}: PortalCotacoesPaginationProps) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-gray-100 bg-white">
      <span className="text-xs font-medium text-gray-500">
        ...
        <strong className="text-gray-900 font-bold">{totalRecords}</strong> cotações
      </span>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <div className="hidden sm:flex items-center gap-1">
          {getVisiblePages().map((pageItem, idx) => {
            ...
            return (
              <Link
                key={`page-${pageItem}`}
                href={createPageUrl(Number(pageItem))}
                className={`min-w-[38px] min-h-[38px] flex items-center justify-center rounded-xl text-xs font-bold transition-colors ${
                  isCurrent
        ...
        <span className="sm:hidden px-3 text-xs font-bold text-gray-700">
