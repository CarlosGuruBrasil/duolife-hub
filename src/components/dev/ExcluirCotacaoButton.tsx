...
interface ExcluirCotacaoButtonProps {
  cotacaoId: string;
  clientName?: string;
...
export function ExcluirCotacaoButton({
  cotacaoId,
  clientName,
  variant = 'table',
  redirectTo,
  onDeleted,
}: ExcluirCotacaoButtonProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      const res = await fetch(`/api/admin/cotacoes/${encodeURIComponent(cotacaoId)}`, {
        method: 'DELETE',
      });
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Erro ao excluir a cotação.');
      }
      ...
      toast.error(err instanceof Error ? err.message : 'Falha na exclusão da cotação.');
    } finally {
      setLoading(false);
    }
  }
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-gray-900">Excluir Cotação</h3>
                <span className="bg-amber-100 text-amber-800 text-[10px] font-extrabold px-1.5 py-0.5 rounded border border-amber-200">
                  DEV
                </span>
              </div>
              <p className="text-xs text-gray-500 mt-1">
                {clientName ? `Cotação de ${clientName}` : `Cotação #${cotacaoId.slice(0, 8)}`}
              </p>
            </div>
          </div>

          <div className="bg-rose-50/80 border border-rose-200/80 rounded-xl p-3 text-xs text-rose-950 space-y-1.5">
            <p className="font-bold text-rose-900">Regras de Exclusão em Cascata:</p>
            <ul className="list-disc list-inside space-y-1 text-rose-800 text-[11px]">
              <li>A cotação será removida permanentemente do banco de dados.</li>
              <li>Todas as <strong>cobranças não pagas vinculadas serão canceladas no Asaas</strong> e excluídas.</li>
              <li>Se a cotação possuir cobranças já compensadas/pagas, a exclusão será bloqueada por segurança.</li>
              <li>Documentos ZapSign vinculados serão desassociados.</li>
            </ul>
          </div>
    ...
    return (
      <>
        <button
          ...
          title="Excluir cotação e cobranças não pagas (Dev)"
        >
          <Trash2 size={14} />
        </button>
        {modal}
    ...
    return (
      <>
        <button
          type="button"
          onClick={() => setIsOpen(true)}
          className="inline-flex items-center gap-1.5 px-3 py-2 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-bold rounded-xl text-xs transition-colors shadow-2xs"
          title="Excluir cotação e cobranças não pagas (Dev)"
        >
          <Trash2 size={13} />
